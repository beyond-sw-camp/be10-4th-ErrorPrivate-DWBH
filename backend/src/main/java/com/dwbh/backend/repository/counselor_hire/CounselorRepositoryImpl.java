...
public class CounselorRepositoryImpl implements CounselorCustomRepository {
    ...
    @Override
    public Page<CounselorDTO> findAllUserCounselListJoinUser(Long userSeq, Pageable pageable) {
        QCounselorType counselorType = new QCounselorType("counselorType");
        QueryResults<CounselorDTO> results = queryFactory.select(
                        Projections.constructor(
                                CounselorDTO.class,
    ...
    private BooleanExpression userSeqEq(Long userSeq) {
        return Optional.ofNullable(userSeq).orElse(0L) == 0 ? null : user.userSeq.eq(userSeq);
    }
